...
eval_df <- eval_df %>%
  mutate(
    Final_01 = (Final_Score - min(Final_Score)) / (max(Final_Score) - min(Final_Score)) # Normalize Final_Score
  )

mae  <- mean(abs(eval_df$S_exam - eval_df$Final_01))
rmse <- sqrt(mean((eval_df$S_exam - eval_df$Final_01)^2))
